# ============================================================
# AXC v2.5 - Fase 4 - Paso 2e - Reporte sin barra de filtros
# ============================================================
# Reemplaza el bloque AXC_REPORTE existente por la version paso2e
# (paso2b sin la barra de filtros Desde/Hasta).
#
# - Detecta si Chart.js esta en <head>; si no, lo inyecta.
# - Idempotente
#
# Backup: .pre_fase4_paso2e.bak
#
# Uso:
#   powershell -ExecutionPolicy Bypass -File C:\AXC\aplicar_fase4_paso2e.ps1
# ============================================================
...
$backup = "$rutaCompleta.pre_fase4_paso2e.bak"

$MarcadorChartInicio = '<!-- AXC Chart.js CDN - inicio -->'
$MarcadorChartFin = '<!-- AXC Chart.js CDN - fin -->'
$MarcadorReporteInicio = '<!-- AXC_REPORTE Fase 4 Paso 2 - inicio -->'
$MarcadorReporteFin = '<!-- AXC_REPORTE Fase 4 Paso 2 - fin -->'

$ChartCdnUrl = 'https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js'

Write-Host ''
Write-Host '=== AXC v2.5 - Fase 4 Paso 2e - Reporte sin filtros locales ===' -ForegroundColor Cyan
Write-Host ''
...
Write-Host "Backup -> $($Archivo).pre_fase4_paso2e.bak" -ForegroundColor DarkGray
...
if ($contenido.Contains($MarcadorReporteInicio)) {
    ...
    Write-Host '  Bloque AXC_REPORTE previo removido' -ForegroundColor DarkCyan
}
...
Write-Host '  AXC_REPORTE paso2e inyectado antes de </body>' -ForegroundColor Green
...
Write-Host '     - SIN barra de filtros Desde/Hasta'
Write-Host '     - 6 cards directamente debajo del titulo'
Write-Host '     - Donut + Top 5'
Write-Host ''
Write-Host '=== Para revertir ===' -ForegroundColor DarkGray
Write-Host "  Copy-Item $($Archivo).pre_fase4_paso2e.bak $Archivo -Force"

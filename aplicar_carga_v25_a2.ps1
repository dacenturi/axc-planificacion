...
Write-Host ''
Write-Host '=== AXC v2.5 - Carga v25 - Commit A.2 (refactor con AXC_BL oficial) ===' -ForegroundColor Cyan
...
if (-not $contenidoJs.Contains("'2.5.3-cargaA2'")) {
    Write-Host 'ERROR: axc_carga_v25.js no es la version cargaA2' -ForegroundColor Red
    exit 1
}
Write-Host "JS A.2: $($contenidoJs.Length) caracteres" -ForegroundColor Green
...
if ($contenido.Contains($MarcadorInicio)) {
    ...
    Write-Host '  Bloque A.1 reemplazado por A.2' -ForegroundColor DarkCyan
}
...
Write-Host '     [AXC_CARGA_V25] cargado v2.5.3-cargaA2'
Write-Host '  3. Visualmente:'
Write-Host '     - Estado: chip con etiquetas oficiales (En proceso, Terminado, etc.)'
Write-Host '     - Filas con cumplimiento real muestran estado correcto (no Sin empezar)'
Write-Host '     - Razones intacto'
Write-Host '     - Dias atraso: numero o 0'
Write-Host '  4. Validacion:'
Write-Host '     window.AXC_CARGA_V25._v'
Write-Host '     // Validar que usa AXC_BL oficial:'
Write-Host '     window.AXC_CARGA_V25.calcularEstado(acts[2])'

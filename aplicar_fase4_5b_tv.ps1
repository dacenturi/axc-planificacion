# ============================================================
# AXC v2.5 - Fase 4.5b TV - Curva S en Panel TV
# ============================================================
# Inyecta dos JS al axc_dashboard_tv.html:
#   1. axc_curva_s.js: extension del modulo 9 (mismo modulo que
#      ya usa el Reporte Global)
#   2. axc_tv_curvas.js: integrador para Panel TV (inserta el
#      septimo card y hookea renderKPIs)
#
# Idempotente. Backup: .pre_fase4_5b_tv.bak
# ============================================================
...
$backup = "$rutaCompleta.pre_fase4_5b_tv.bak"

$M1Inicio = '<!-- AXC_CurvaS modulo TV - inicio -->'
$M1Fin = '<!-- AXC_CurvaS modulo TV - fin -->'
$M2Inicio = '<!-- AXC_TV_CURVAS integrador - inicio -->'
$M2Fin = '<!-- AXC_TV_CURVAS integrador - fin -->'

Write-Host ''
Write-Host '=== AXC v2.5 - Fase 4.5b TV - Curva S en Panel TV ===' -ForegroundColor Cyan

if (-not (Test-Path $JsCurva)) {
    Write-Host "ERROR: No se encuentra $JsCurva" -ForegroundColor Red
    Write-Host '  Aplicar primero el modulo Curva S desde Reporte Global' -ForegroundColor Yellow
    exit 1
}
...
if (-not $contenidoTv.Contains("'2.5.2-paso4.5b-tv'")) {
    Write-Host 'ERROR: axc_tv_curvas.js no es la version 2.5.2-paso4.5b-tv' -ForegroundColor Red
    exit 1
}

Write-Host "Curva S modulo: $($contenidoCurva.Length) caracteres" -ForegroundColor Green
Write-Host "Integrador TV: $($contenidoTv.Length) caracteres" -ForegroundColor Green

Copy-Item $rutaCompleta $backup -Force
Write-Host "Backup -> $($Archivo).pre_fase4_5b_tv.bak" -ForegroundColor DarkGray
...
if ($contenido.Contains($M1Inicio)) {
    ...
    Write-Host '  Bloque modulo Curva S TV previo removido' -ForegroundColor DarkCyan
}

# ---- Bloque 2: integrador TV ----
$bloque2 = $M2Inicio + "`r`n<script>`r`n" + $contenidoTv + "`r`n</script>`r`n" + $M2Fin + "`r`n"
if ($contenido.Contains($M2Inicio)) {
    $idxI = $contenido.IndexOf($M2Inicio)
    $idxF = $contenido.IndexOf($M2Fin) + $M2Fin.Length
    if ($idxF -lt $contenido.Length -and $contenido.Substring($idxF, 2) -eq "`r`n") { $idxF += 2 }
    $contenido = $contenido.Remove($idxI, $idxF - $idxI)
    Write-Host '  Bloque integrador TV previo removido' -ForegroundColor DarkCyan
}

# Inyectar ambos antes de </body>
...
Write-Host '     [AXC_TV_CURVAS] cargado v2.5.2-paso4.5b-tv'
Write-Host '     [AXC_TV_CURVAS] hook a renderKPIs instalado'
Write-Host '  3. Aparece septimo card "Curva S - Planificado vs Real" en la grilla del TV'
Write-Host '  4. El badge muestra el % avance real'
